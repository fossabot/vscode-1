...
function asyncableGetCollections(activeConnection: any, databaseName: string, ) {
...
export default class SunburstController {
  constructor(connectionController: ConnectionController) {
    ...
    activeConnection.listDatabases(async (err: any, databases: any) => {
      if (err) {
        vscode.window.showErrorMessage(`Error fetching databases for sunburst: ${err}`);
        return;
      }
      ...
      try {
        ...
        if (collectionsErr) {
          vscode.window.showErrorMessage(`Error fetching database collections for sunburst view: ${err}`);
          return;
        }
      }
